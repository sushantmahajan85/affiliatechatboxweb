"use client";
import { Bell, Check, MoreHorizontal, Filter, Search } from "lucide-react";
...
export function NotificationsPage() {
  ...
  const { data, isLoading, isFetching } = useGetNotificationsPaginatedQuery(
    {
      userId: user?._id || "",
      page,
      limit: PAGE_SIZE,
  ...
  useEffect(() => {
    if (!data) return;
    const batch = data.notifs || [];
    setNotifications((prev) => {
      if (page === 1) return batch;
      const seen = new Set(prev.map((n) => n._id));
      return [...prev, ...batch.filter((n) => !seen.has(n._id))];
    });
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      ...
      setNotifications((prev) =>
        prev.map((notif) =>
          notif.type === "chat_request" ? notif : { ...notif, isRead: true }
        )
      );
    } catch {
  ...
  const showInitialLoading =
    notifications.length === 0 && (isLoading || isFetching) && page === 1;
          ...
          {showInitialLoading ? (
            ...
            <>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-1 gap-2">
                      <div>
                        <h4
                          className={`text-[15px] font-bold text-[#1A1A2E] ${!notif.isRead ? "font-black" : ""}`}
                        >
                          {notif.title}
                        </h4>
                      </div>
                    ...
                    <p className="text-sm text-[#757575] leading-relaxed">{notif.message}</p>
                  </div>
                </motion.div>
              ))}
              ...
              {isFetching && page > 1 && (
                <div className="p-6 text-center">
                  <div className="w-6 h-6 border-2 border-[#7B61FF]/20 border-t-[#7B61FF] rounded-full animate-spin mx-auto" />
                </div>
              )}
            </>
          ) : (
            <div className="p-20 text-center">
              ...
              <p className="text-sm text-[#94A3B8]">
                We&apos;ll alert you when something interesting happens.
              </p>
            </div>
          )}
        </div>
